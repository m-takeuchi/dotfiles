...
" By Takeuchi
" toggle @now context tag on a task
function! s:ToggleNow()
    if (getline(".") =~ '^\s*- ')
        let isnow = strridx(getline("."),"@now")
        if (isnow != -1)
            substitute/ @now//
            echo "unnow!"
        else
...
" By Takeuchi
noremap <unique> <script> <Plug>ToggleNow       :call <SID>ToggleNow()<CR>
...
" By Takeuchi
map <buffer> <silent> <LocalLeader>td <Plug>ToggleNow
